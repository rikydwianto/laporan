...
while($bulan = mysqli_fetch_array($qbulan)){
    ...
    <table class='table table-bordered'>
        ...
        <?php 
        $mon = sprintf("%02d",$mon);
        $qsa = mysqli_query($con,"select * from surat s join karyawan k on s.id_karyawan=k.id_karyawan where s.id_cabang='$id_cabang' and tgl_surat like '$tahun-$mon-%' order by s.no_urut desc");
        while($surat = mysqli_fetch_array($qsa)){
            ?>
            <tr>
                <td><?=$no++?></td>

	...
	<div class="panel-body post-body table-responsive ">
		...
		<table class='table table-bordered'>
			...
			while ($tampil = mysqli_fetch_assoc($cek_ka)) {
				...
				$qqq = "SELECT sum(detail_laporan.total_agt)as anggota,sum(detail_laporan.member)as member, sum(detail_laporan.total_bayar)as bayar,sum(detail_laporan.total_tidak_bayar)as tidak_bayar,count(no_center) as hitung_center, laporan.* FROM laporan JOIN detail_laporan on laporan.id_laporan=detail_laporan.id_detail_laporan JOIN karyawan on laporan.id_karyawan=karyawan.id_karyawan where laporan.tgl_laporan='$qtgl' and karyawan.id_karyawan='$tampil[id_karyawan]' ";
				$cek_l = mysqli_query($con, "$qqq");

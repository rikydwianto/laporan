<div class="row">
    ...
    if (isset($_POST['kirim'])) {
        ...
        $alasan = $_POST['alasan'];
        $penyelesaian_par = $_POST['tgl'];
        $loan = $_POST['loan'];
        $hitung = count($id_nasabah);
        for ($i = 0; $i < $hitung; $i++) {
            $cek = mysqli_query($con, "select * from alasan_par where id_cabang='$id_cabang' and id_loan='$loan[$i]' ");
            if (mysqli_num_rows($cek)) {
                $update = mysqli_query($con, "update alasan_par set penyelesaian_par='$penyelesaian_par[$i]',alasan='$alasan[$i]' where id_cabang='$id_cabang' and id_loan='$loan[$i]'");
            } else {
                $update = mysqli_query($con, "INSERT INTO `alasan_par` (`alasan`, `id_loan`, `id_detail_nasabah`, `id_cabang`, `id_karyawan`,`penyelesaian_par`) 
            VALUES ('$alasan[$i]', '$loan[$i]', '$id_nasabah[$i]', '$id_cabang', '$id_karyawan','$penyelesaian_par[$i]') 
            ");
                echo "insert";
            }
        }
        ...
        <div class="col-md-6">
            <form method="get">
                ...
                <select name="ctr" class='form-control' required id="">
                    ...
                    <?php
                    $ctr = $_GET['ctr'];
                    $tgl = mysqli_fetch_assoc(mysqli_query($con, "select max(tgl_input) as tgl from deliquency where id_cabang='$id_cabang'"))['tgl'];
                    ...
                    while ($ce = mysqli_fetch_assoc($q)) {
                        if ($ce['no_center'] == $ctr) $ak = 'selected';
                        else $ak = '';
                    ?>
                        <option <?= $ak ?> value="<?= $ce['no_center'] ?>">CENTER <?= $ce['no_center'] ?> -
                            <?= $ce['hari'] ?> ||
                            <?= $ce['total'] ?> client</option>
                    <?php
                    }
                    ?>
                </select>

                <input type="submit" value="FILTER" class='btn btn-danger mr-2'>
            ...
            <table class='table'>
                ...
                while ($row = mysqli_fetch_assoc($q)) {
                    ...
                    if (mysqli_num_rows($qr)) {
                        $reason = mysqli_fetch_assoc($qr);
                        $ket = $reason['alasan'];
                        $a = $ket;
                        $tgl_target = $reason['penyelesaian_par'];
                    } else {

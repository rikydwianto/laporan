...
<div class="col-md-12">
	<div class="panel-body post-body table-responsive ">
		...
		<table class='table'>
			...
			?>
						<tr>
							...
							<td>
								<?php
								while ($ket = mysqli_fetch_assoc($cek_l1)) {
		...
		<a href="<?= $url . $menu ?>rekap_laporan_minggu&grafik&bayar=<?= $hitung_bayar ?>&member=<?= $hitung_member ?>&client=<?= $hitung_agt ?>&persen=<?= $persen ?>&tgl=<?= $tglawal ?>&tgl1=<?= $tglakhir ?>&dtd=<?= $hitung_dtd ?>" class="btn btn-danger" onclick="return window.confirm('Apakah Sudah benar???')">Simpan Ke Grafik</a>
		...
		*** TIDAK DAPAT DIEDIT
		...
		<table class="table">
			...
			while ($r = mysqli_fetch_assoc($cek_ket)) {
			?>
				<tr>
					<td><?= $no1++ ?></td>

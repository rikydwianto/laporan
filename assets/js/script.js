...
$.getJSON(url_link + "api/cabang.php", function (data) {
  ...
  $.each(data, function (i, field) {
    if (data[i]["latitude"] == null) {
      ...
      L.marker([data[i]["latitude"], data[i]["longitude"]], { icon: kantor })
        .addTo(map)
        .bindPopup(
          "<h4>KANTOR CABANG - " +
            data[i]["nama_cabang"].toUpperCase() +
            "</h4>"
        );
    }
  });
...
$.getJSON(url_link + "api/center.php", function (data) {
  ...
  $.each(data, function (i, field) {
    ...
    L.marker([data[i]["latitude"], data[i]["longitude"]], { icon: ikon_center })
      .addTo(map)
      .bindPopup(isi);
  });
});
...
$.getJSON(url_link + "api/peta.php", function (data) {
  ...
  $.each(data, function (i, field) {
    ...
    L.marker([data[i]["latitude"], data[i]["longitude"]], { icon: ikon })
      .addTo(map)
      .bindPopup(text);
  });
});
...
map.on("click", function (e) {
  ...
  text +=
    "<br><a class='btn' href='" +
    url_link +
    "index.php?menu=lokasi&pilih=center" +
    "&lat=" +
    e.latlng.lat +
    "&lng=" +
    e.latlng.lng +
  ...
  text +=
    "<br><a class='btn' href='" +
    url_link +
    "index.php?menu=lokasi&pilih=anggota" +
    "&lat=" +
    e.latlng.lat +
    "&lng=" +
    e.latlng.lng +
  ...
  text +=
    "<br><a class='btn' href='" +
    url_link +
    "index.php?menu=lokasi&pilih=pu" +
    "&lat=" +
    e.latlng.lat +
    "&lng=" +
    e.latlng.lng +
  ...
  var titik = L.marker([e.latlng.lat, e.latlng.lng])
    .addTo(map)
    .bindPopup(text)
    .openPopup();

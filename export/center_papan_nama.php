...
<body> 
<h1 style='text-align:center'>JADWAL CENTER MEETING <br/> CABANG <?= strtoupper($d['nama_cabang']) ?> <br/> DENGAN JAM DAN NAMA CENTER<br/></h1>
	<table border="1">
    ...
    <tbody>
           ...
           <?php $qkar = mysqli_query($con,"SELECT distinct k.nama_karyawan,k.id_karyawan from center c join karyawan k on k.id_karyawan=c.id_karyawan where c.id_cabang='$id_cabang' order by k.nama_karyawan asc ");
                while($kar=mysqli_fetch_array($qkar)){
                    $qcenter = mysqli_query($con,"SELECT no_center,status_center,member_center,jam_center,desa,kecamatan,nama_center from center where id_cabang='$id_cabang' and hari='$hari[hari]' and id_karyawan='$kar[id_karyawan]' order by jam_center asc");
                    ?>
                    <td style="vertical-align: top; font-size:10px; ">
                       ...
                       while($center=mysqli_fetch_array($qcenter)){
                        ...
                        if($status=='hijau') $warna='#025e1e';
                            ...
                            echo "<b style='color:$warna; padding-top:0px;'>$jam | ".sprintf("%03d", $center['no_center'])."  ".'<br/>'.'  '.strtoupper(str_replace(" "," ",$center['nama_center']))."<br/> client : $center[member_center] </b>"."<hr/>";
